...
describe('Befunge', function () {
    ...
    describe('#divide()', function () {
        ...
        it('should give only integers', function () {
            bef.stack = [3, 7];
            bef.divide();
            expect(bef.stack).to.deep.equal([2]);
        });
    ...
    describe('#outInt()', function () {
        it('should pop last stack value and write int value to stdout', function () {
            bef.stack = [1, 2, 3];
            bef.outInt();
            expect(hook.captured()).to.equal("3\n");
            bef.outInt();
            expect(hook.captured()).to.equal("3\n2\n");
        });
    });
    describe('#outAscii()', function () {
        it('should pop last stack value and write ascii char to stdout', function () {
            bef.stack = [67, 99];
            bef.outAscii();
            expect(hook.captured()).to.equal("c\n");
            bef.outAscii();
            expect(hook.captured()).to.equal("c\nC\n");
        });
    });
    ...
    describe('#put()', function () {
        it('should write value to program cell', function () {
            bef.stack = [10, 3, 6];
            bef.put();
            expect(bef.program[6][3]).to.equal(10);
        });
    });
    describe('#get()', function () {
        it('should get value from program cell', function () {
            bef.program[3][2] = 8;
            bef.stack = [2, 3];
            bef.get();
            expect(bef.stack).to.deep.equal([8])
        });
    });
    describe('#inInt()', function () {
        it('should read value from stdin as int'
            // , function () {
            // bef.inInt();
            // stdin.send('3', 'ascii');
            // stdin.end();
    ...
    describe('#inAscii()', function () {
        it('should read value from stdin as ascii');
    });
